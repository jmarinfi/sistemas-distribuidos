...
public class ActivitySimulation extends TimerTask{
	...
	public void run(){
		...
		if(simulationData.isConnected()){
			// JMF: Lower probability of disconnection if there are pending operations to replicate
			double adjustedProbDisconnect = serverData.hasPendingOperations() ? simulationData.getProbDisconnect() / 2 : simulationData.getProbDisconnect();
			
			//probability of disconnection
			if(a<adjustedProbDisconnect){
			...
			if(a>=adjustedProbDisconnect
					&& a<adjustedProbDisconnect+simulationData.getProbCreate()
			){
			...
			if(simulationData.deletionActivated()
					&& a>=adjustedProbDisconnect+simulationData.getProbCreate()
					&& a<adjustedProbDisconnect+simulationData.getProbCreate()+simulationData.getProbDel()
			){
