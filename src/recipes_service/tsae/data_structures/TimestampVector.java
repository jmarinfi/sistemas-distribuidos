...
//	private transient LSimWorker lsim = LSimFactory.getWorkerInstance();
	...
	public synchronized void updateTimestamp(Timestamp timestamp){
		...
		if (timestamp != null && timestampVector.containsKey(timestamp.getHostid())) {
			Timestamp currentTimestamp = timestampVector.get(timestamp.getHostid());
			if (currentTimestamp.compare(timestamp) < 0) {
	...
	public synchronized void updateMax(TimestampVector tsVector){
		if (tsVector != null) {
			for (String key : tsVector.timestampVector.keySet()) {
				Timestamp otherTs = tsVector.timestampVector.get(key);
				Timestamp thisTs = timestampVector.get(key);
				if (thisTs.compare(otherTs) < 0) {
					timestampVector.put(key, otherTs);
				}
			}
		}
	}
	...
	public synchronized TimestampVector clone(){
		TimestampVector clonedTs = new TimestampVector(Arrays.asList(timestampVector.keySet().toArray(new String[0])));
		clonedTs.timestampVector = new ConcurrentHashMap<>(timestampVector);
		return clonedTs;
	}
