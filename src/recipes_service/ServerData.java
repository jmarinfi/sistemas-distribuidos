...
public class ServerData {
	...
	List<RemoveOperation> recipesPendingToRemove = new Vector<RemoveOperation>();
	
	// end: true when program should end; false otherwise
	private boolean end;
	
	// JMF: pendingOperations: true if there are pending operations to execute
	private boolean pendingOperations = true;
...
//		System.err.println("Error: removeRecipe method (recipesService.serverData) not yet implemented");
		Recipe recipe = getRecipes().get(recipeTitle);
		Timestamp opTimestamp = nextTimestamp();
		RemoveOperation op = new RemoveOperation(recipeTitle, recipe.getTimestamp(), opTimestamp);
		
		getLog().add(op);
		getSummary().updateTimestamp(opTimestamp);
		tombstones.add(recipe.getTimestamp());
		recipesPendingToRemove.add(op);
	}
	...
	public synchronized void executeOperation(Operation op) {
		if (op.getType() == OperationType.ADD) {
			...
			for (RemoveOperation remOp: recipesPendingToRemove) {
				Recipe recipeRemOp = recipes.get(remOp.getRecipeTitle());
				if (recipeRemOp != null && addOp.getRecipe().equals(recipeRemOp) && addOp.getTimestamp().compare(remOp.getTimestamp()) > 0) {
					isMarkedAsRemoved = true;
				}
			}
			
			if (!isMarkedAsRemoved && !recipes.contains(addOp.getRecipe().getTitle())) {
				recipes.add(addOp.getRecipe());
			}
		}
		else if (op.getType() == OperationType.REMOVE) {
			RemoveOperation remOp = (RemoveOperation) op;
			if (!tombstones.contains(remOp.getRecipeTimestamp())) {
				tombstones.add(remOp.getRecipeTimestamp());
			}
			if (!recipesPendingToRemove.contains(remOp)) {
				recipesPendingToRemove.add(remOp);
			}
		}
	}

	public void executeOperations(List<Operation> opsReceived) {
		for (Operation op: opsReceived) {
			executeOperation(op);
		}
		pendingOperations = false;
		purgeTombstones();
		for (RemoveOperation remOp: recipesPendingToRemove) {
			if (!tombstones.contains(remOp.getRecipeTimestamp()) && recipes.contains(remOp.getRecipeTitle())) {
				recipes.remove(remOp.getRecipeTitle());
			}
		}
	}
	...
	public void setPendingOperations(boolean pendingOperations) {
